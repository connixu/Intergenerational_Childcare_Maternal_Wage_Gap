...
NLSY_Valid <- NLSY_imputed %>% subset(YEAR>=2005) %>% 
  ...
  filter(CV_HGC_EVER_EDT >= 12) %>% subset(INCOME>0) 

# Step 5. Make df that includes working women with valid data about childcare
NLSY_Valid_Childcare <- NLSY_Valid %>% subset(INCOME>0) %>% subset(CV_HH_UNDER_6>0) %>%
  subset(SELF_CARE==0) %>% subset(PART_TIME==1)
...
summary(
  glm(FULL_TIME ~ FAMILY_CARE + BA_ABOVE + MARRIED_OR_COHABITATING+N_CHILDREN,data =NLSY_Valid_Childcare, family = "binomial") ## this is equivalent to above OLS ##
        )
```
